...
echo "check visibilities of quads in revisions HEAD~5: empty revision" > ${INFO_OUTPUT}
...
result=$(curl_graph_store_get_code_nofail --repository mem-rdf-revisioned revision-id=HEAD~5 2>&1 > /dev/null)
test "$result" -eq "404"
echo "check visibilities of quads in revisions HEAD~4" > ${INFO_OUTPUT}
curl_graph_store_get --repository mem-rdf-revisioned revision-id=HEAD~4 | tr -s '\n' '\t' \
    | fgrep    "object-3.3" | fgrep -v "object-3.4" | fgrep -v "object-3.4-extra" | fgrep -v "object-foo" > ${GREP_OUTPUT}
